  ...
  const handleRegister = async () => {
    ...
    try {
      if (!(email && firstName && lastName)) {
        ...
        setSnackbarMessage(
          "Please enter all the required information to register"
        );
        return;
      }
      ...
      const response =
        mode == "customer"
          ? await RegisterCustomerAPI(bodyParameters)
          : await RegisterPartnersAPI(bodyParameters);
      console.log("Registration response:", response);
      if (response.status === 201) {
        setOpenSnackbar(true);
        setSnackbarMessage(
          "Registration successful! Please sign in with your email"
        );
  ...
  const handleEmailSignIn = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        setOpenSnackbar(true);
        setSnackbarMessage(
          "A login link has been sent to your registered email address. Please check your inbox."
        );
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (response) => {
      try {
        ...
        localStorage.setItem("AUTH_ACCESS_TOKEN", response.access_token); // Save user details or token to localStorage/sessionStorage
